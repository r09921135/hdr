...
class HDR(Weights):
    ...
    def solve_hdr(self, images, hdr_method, ln_st, n_samples, n_epochs, wtype):
        ...
        if hdr_method == 'debevec':
            breakpoint()
            samples_bgr = self.sample_pixels(images, n_samples)  # len(samples_bgr) = c, len(samples_bgr[0]) = n_images, samples_bgr[0][0].shape = n_samples
    ...
    def solve(self, indir, savedir, 
        ...
        # read images
        images = self.read_images(indir)  # type(images) = list, images[0].shape = (h, w, c) 
        n_images = len(images)
        ...
        lnG_bgr = self.solve_hdr(images, hdr_method, ln_st, n_samples, n_epochs, wtype)  # len(lnG_bgr) = c, lnG_bgr[0].shape = (256)
        
        # reconstruct radiance
        radiance_bgr = self.compute_radiance(images, lnG_bgr, ln_st, wtype, savedir)  # radiance_bgr.shape = (h, w, c)
