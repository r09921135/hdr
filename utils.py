...
class Weights():
    ...
    def get_weights(self, Z, wtype='triangle', mean=128, sigma=128):
        if wtype == 'triangle':
            weights = np.concatenate((np.arange(1, 129), np.arange(1, 129)[::-1]), axis=0)  # e.q: [1,2,...128,128,127,...,1]
...
class ImageAlignment():
    ...
    def median_threshold_bitmap(self, image, percentile=50, _print=False):
        image_rbg_sum = np.sum(image, axis=2)
        image_threshold = np.percentile(image_rbg_sum, percentile)
        image_boolean = ( image_rbg_sum >= image_threshold )*255
        if _print :
        ...
        return image_boolean.astype(np.float32)
    ...
    def fit(self, src, tar, depth=4):
        h, w, c = tar.shape
        # src_mtb = self.median_threshold_bitmap(src)
        # tar_mtb = self.median_threshold_bitmap(tar)
...
class DebevecMethod(Weights):
    ...
    def construct_matrix(self, Z, ln_st, N, P, wtype, constraint=127, lamda=10):
        cols = np.arange(N * P)  # cols.shape = (N*P)
        rows = np.array(Z).flatten()  # rows.shape = (N*P)
...
class ToneMapping():
    ...
    def gaussian_blurs(self, im, smax=25, a=0.5, fi=8.0, epsilon=0.01):
        ...
        blur_list = np.zeros(im.shape + (num_s,))  # blur_list.shape = (h, w, num_s)
        ...
        smax = np.argmax(Vs_list > epsilon, axis=2)  # smax.shape = (h, w)
        smax[np.where(smax == 0)] = num_s  # clamp to num_s
        smax -= 1
        
        # select blur size for each pixel
        print(', apply index...')
        I, J = np.ogrid[:cols, :rows]  # I.shape = (h, 1), J.shape = (1, w)
    ...
    def durand_bilateral(self, hdr):
        ...
        Lw = (hdr[:, :, 0] * b + hdr[:, :, 1] * g + hdr[:, :, 2] * r) / (b + g + r)  # obtain luminance
        log_Lw = np.log10(Lw)
        log_base = cv2.bilateralFilter(log_Lw, 5, 15, 15)
        log_detail = log_Lw - log_base
        
        cf = dR / (np.max(log_base) - np.min(log_base))  # compression factor
        # if dR = 1, the value of cf * (log_base - np.max(log_base)) would be [-1, 0]
        log_Ld = cf * (log_base - np.max(log_base)) + log_detail
        Ld = np.power(10, log_Ld)  # reconstruct the log10 intensity

        ldr[:, :, 0] = (hdr[:, :, 0] / Lw) * Ld
        ldr[:, :, 1] = (hdr[:, :, 1] / Lw) * Ld
        ldr[:, :, 2] = (hdr[:, :, 2] / Lw) * Ld

        ldr = np.clip((ldr ** 0.3) * 255, 0, 255)  # gamma mapping
